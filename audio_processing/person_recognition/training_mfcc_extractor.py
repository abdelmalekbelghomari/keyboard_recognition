...
def extract_mfccs_to_csv(audio_file, label, output_csv, n_mfcc, n_segments=20):
    """
    Extract MFCCs from a single audio file, adjusting the segment length based on the audio file's total length,
    aiming to produce a fixed number of features (MFCCs).
...
def main():
    parser = argparse.ArgumentParser(description="Extract MFCCs from a WAV file to a CSV file, adjusting the number of segments based on audio file's length.")
    ...
    extract_mfccs_to_csv(args.audio_file, args.label, args.output_csv, args.n_mfcc, args.n_segments)
